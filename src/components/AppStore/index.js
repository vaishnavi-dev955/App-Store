...
class AppStore extends Component {
  ...
  render() {
    const getFilteredapps = this.getFilteredapps()
    const {activeTabId, searchInput} = this.state
    const filteredAppList = getFilteredapps.filter(each =>
        ...
        <ul className="tabsList-container">
          {tabsList.map(eachItem => (
            <TabItem
              tabDetails={eachItem}
              key={eachItem.tabId}
              OnClickTabGetCorrespondingAppps={
                this.OnClickTabGetCorrespondingAppps
              }
